...
import { baseHandler } from '../../../../lib/game/functions/lambda/listGamesByOpponentIdHandler.js';
...
describe('list events by opponent id handler', () => {
  const findGameSpy = vi.spyOn(Game, 'findByCurrentOpponentId');
  ...
  test('list events', async () => {
    findGameSpy.mockResolvedValue([games[1]]);
